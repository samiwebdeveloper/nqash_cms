<?php
class InvoiceRulesModel extends CI_Model {

    public function getInvoiceRulesListByCustomer($customerId){
        //ir.invoice_rule_json, ir.invoice_rule_sql, 
        $query="SELECT ir.customer_id,
                    ir.invoice_rule_id, ir.version_no, ir.invoice_rule_name, 
                    
                    usr.oper_user_name as modified_by, ir.modified_date 
                FROM invoice_rules ir 
                INNER JOIN saimtech_oper_user usr 
                ON ir.modified_by = usr.oper_user_id
                WHERE is_active = 1 AND customer_id={$customerId} 
                Order by ir.invoice_rule_name";
        
        //$res = $this->db->query($query,array($customerId));
        $res = $this->db->query($query);
        
        return $res->result();
    }
    
    public function getInvoiceRuleDetailsByCustomer($customerId, $invoice_rule_id){
        //ir.invoice_rule_json, ir.invoice_rule_sql,
        $query="SELECT ir.customer_id,
                    ir.invoice_rule_id, ir.version_no, ir.invoice_rule_name,
                    ir.invoice_rule_json, ir.invoice_rule_sql,
                    ir.modified_by, ir.modified_date
                FROM invoice_rules ir
                WHERE is_active = 1 AND customer_id={$customerId} AND invoice_rule_id={$invoice_rule_id}";
    ...
    public function saveCustomerInvoiceRuleDetails($customer_id, $invoice_rule_id, $version_no, $invoice_rule_name, $invoice_rule_json, $invoice_rule_sql, $modified_by){
        
        //if($version_no == -1){
            
            $procedure = "CALL save_invoice_rule (?, ?, ?, ?, ?, ?, ?)";
            
            $res = $this->db->query($procedure, array('invoice_rule_id'=>$invoice_rule_id, 'customer_id'=>$customer_id, 'version_no'=>$version_no, 
                'invoice_rule_name'=>$invoice_rule_name, 'invoice_rule_json'=>$invoice_rule_json, 'invoice_rule_sql'=>$invoice_rule_sql, 
    ...
    public function getActiveCustomer(){
        ...
        $query="SELECT saimtech_customer.customer_id as id,saimtech_customer.customer_name as name,city_name as city,is_gst
        FROM saimtech_customer
        INNER JOIN saimtech_city ON saimtech_city.city_id=saimtech_customer.customer_city
        WHERE  saimtech_customer.is_enable = 1
        GROUP BY saimtech_customer.customer_id order by saimtech_customer.customer_name";
        $res = $this->db->query($query);
        return $res->result();
    }
...
$query="SELECT `customer_name`, `is_gst` FROM `saimtech_customer` WHERE `customer_id`=?";
